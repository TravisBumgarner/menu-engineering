  ...
  return (
    <DefaultModal title={`${t('addToRecipe')}: ${recipe.title}`} sx={{ minHeight: '550px' }}>
      <Stack spacing={SPACING.MEDIUM.PX} sx={{ flexGrow: 1 }}>
        ...
        {selectedTab === TABS.AUTOCOMPLETE && <AutocompleteTab recipe={recipe} setTab={handleSetTab} />}
        {selectedTab === TABS.ADD_INGREDIENT && <AddIngredientTab recipe={recipe} />}
        {selectedTab === TABS.ADD_RECIPE && <AddRecipeTab parentRecipe={recipe} />}
      </Stack>
    </DefaultModal>
  )
}
